...
const ResetPasswordPage: React.FC<PropsFromRedux> = (props) => {
    ...
    const [inputValue, setInputValue] = useState({ //For input values
        email: {value: "", touched: false, error: false, errorText: "", name: "EMAIL"},
        code: {value: "", touched: false, error: false, errorText: "", name: "CODE"},
        newPassword: {value: "", touched: false, error: false, errorText: "", name: "NEW_PASSWORD"},
        retypePassword: {value: "", touched: false, error: false, errorText: "", name: "CONFIRM_PASSWORD"},
    });
    ...
    const handleSubmit = (event: React.FormEvent) => {
        ...
        const codeError = validateField(inputValue.code.value, isNotEmpty(inputValue.code.value));
        const newPasswordError = validateField(inputValue.newPassword.value,
        ...
        if(isEmailEntered) {
            ...
            if (newPasswordError[0]) setInputValue(prevStyle => ({
                ...prevStyle,
                newPassword: {...prevStyle.newPassword, error: true, errorText: newPasswordError[0]}
            }));
            if (retypeError[0]) setInputValue(prevStyle => ({
                ...prevStyle,
    ...
    useEffect(() => { //Detect async error status
        switch (props.errorText.code) {
            case "UserNotFoundException": {
                setInputValue(prevStyle => ({
                    ...prevStyle,
                    email: {...prevStyle.email, error: true, errorText: "User not found"}
                }));
                break;
            }
    ...
    const getErrorMessage = () => { //Setting error messages
        ...
        if (inputValue.newPassword.errorText) return inputValue.newPassword.errorText;
        if (inputValue.retypePassword.errorText) return inputValue.retypePassword.errorText;
        return "";
    }
