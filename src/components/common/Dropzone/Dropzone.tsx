...
const FileDropzone: React.FunctionComponent<PropsType> = (props) => {
    ...
    const [fileToDelete, setFileToDelete] = useState<{ index: number; type: string }>({
        index: 0,
        type: '',
    });
        ...
        onDrop: (acceptedFiles) => {
            acceptedFiles.map((item) => {
                if (item.type.includes('video')) {
                    //Set all video files to videos array
                    setVideos((oldArray) => [
                        ...oldArray,
                        Object.assign(item, {
                    ...
                    //Set all image files to photos array
                    setPhotos((oldArray) => [
                        ...oldArray,
                        Object.assign(item, {
                            preview: URL.createObjectURL(item),
                    ...
                    //Set all pdf files to documents array
    ...
    useEffect(() => {
        //Automatically set the first dropped item for preview
    ...
    const deleteFile = (index: number, type: string) => {
        if (type.includes('video')) {
            ...
            if (videos.length === 1) {
                // Set video or pdf to preview if photos is empty after delete
                if (photos.length > 0) {
                    createPreview(0, 'image');
                } else {
        ...
        if (type.includes('image')) {
            ...
            if (photos.length === 1) {
                // Set image or pdf to preview if videos is empty after delete
                if (videos.length > 0) {
                    createPreview(0, 'video');
                } else {
                    if (documents.length > 0) {
        ...
        if (type.includes('pdf')) {
            setDocuments([
                ...documents.slice(0, index).concat(documents.slice(index + 1, documents.length)),
            ]);
            if (documents.length === 1) {
                // Set video or image to preview if documents is empty after delete
                if (photos.length > 0) {
    ...
    const createPreview = (index: number, type: string) => {
        // Function to set preview HTML content
        setContentPreview(
    ...
    const VideosCarouselItems = videos.map((
        videos: any,
        index //Preparing video HTML items for carousel
    ) => (
    ...
    const PhotosCarouselItems = photos.map((
        photo: any,
        index //Preparing image HTML items for carousel
    ) => (
    ...
    const DocumentsCarouselItems = documents.map((
        file: any,
        index //Preparing pdf HTML items for carousel
    ) => (
